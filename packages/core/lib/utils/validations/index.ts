...
function validatePropertyDefinition(prop: unknown): prop is PropertyDefinition {
  ...
  switch (fieldType) {
    ...
    case PropertyType.HEIGHT: {
      if (!isSizeValue(propValue))
        console.warn(`Invalid size value for property type ${fieldType}:`, propValue);
      return isSizeValue(propValue);
    }
    case PropertyType.COLOR:
    case PropertyType.BORDER_COLOR:
    case PropertyType.BACKGROUND_COLOR: {
      if (!isColorValue(propValue))
        console.warn(`Invalid color value for property type ${fieldType}:`, propValue);
      return isColorValue(propValue);
    }
    case PropertyType.BORDER_RADIUS: {
      if (!isBorderRadiusValue(propValue))
        console.warn(`Invalid border radius value for property type ${fieldType}:`, propValue);
      return isBorderRadiusValue(propValue);
    }
    case PropertyType.MARGIN: {
      if (!isMarginValue(propValue))
        console.warn(`Invalid margin value for property type ${fieldType}:`, propValue);
      return isMarginValue(propValue);
    }
    case PropertyType.PADDING: {
      if (!isPaddingValue(propValue))
        console.warn(`Invalid padding value for property type ${fieldType}:`, propValue);
      return isPaddingValue(propValue);
    }
    case PropertyType.BOX_SHADOW: {
      if (!isBoxShadowValue(propValue))
        console.warn(`Invalid box shadow value for property type ${fieldType}:`, propValue);
